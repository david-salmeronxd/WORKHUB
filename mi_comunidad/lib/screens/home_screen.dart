...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadBusinesses() async {
    if (await BusinessService.loadBusinesses() && mounted) {
      await BusinessService.loadFavorites();
      setState(() => businesses = List<Map<String, dynamic>>.from(BusinessService.businessesNotifier.value));
    }
  }
  ...
  Future<void> _toggleFavorite(Map<String, dynamic> store) async {
    await BusinessService.toggleFavorite(store['id'].toString());
    if (!mounted) return;
    final updated = List<Map<String, dynamic>>.from(BusinessService.businessesNotifier.value);
    setState(() {
      businesses = updated;
      favoriteBusinesses = updated.where((business) => business['isFavorite'] == true).toList();
      selectedBusiness = updated.firstWhere(
        (business) => business['id'].toString() == store['id'].toString(),
        orElse: () => store,
      );
    });
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              final deleted = await BusinessService.deleteBusiness(businessId);
                          ...
                          onPressed: isProcessing
                              ...
                              : () async {
                                  if (paymentFormKey.currentState!.validate()) {
                                    ...
                                    final saved = await BusinessService.addBusiness(newBusinessData);
                                    if (saved == null) {
                                      if (mounted) {
                                        setModalState(() => isProcessing = false);
                                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                          content: Text('No se pudo guardar el pago y el negocio.'),
                                        ));
                                      }
                                      return;
                                    }
                                    ...
                                    setState(() {
                                      businesses.add(saved);
                                      selectedBusiness = saved;
                                    });
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        final saved = await BusinessService.addReview(
                        ...
                        if (!mounted) return;
                        if (!saved) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No se pudo guardar la reseña')));
                          return;
                        }
                        final updated = List<Map<String, dynamic>>.from(BusinessService.businessesNotifier.value);
                        final selectedId = selectedBusiness!['id'].toString();
                        setState(() {
                          businesses = updated;
                          selectedBusiness = updated.firstWhere((business) => business['id'].toString() == selectedId);
                        });
                        Navigator.pop(context);
          ...
          FlutterMap(
            ...
            children: [
              TileLayer(
                ...
                userAgentPackageName: 'com.example.mis_servicios',
              ),
              MarkerLayer(
                markers: [
                  Marker(
